...
| `OPEN_ROUTER_API_KEY` | OpenRouter key for the AI assistant (`app-env` secret) |
| `TELEGRAM_BOT_TOKEN` | Telegram bot token (`app-env` secret) |

`DB_ADMIN_PASSWORD` rules: 12–30 chars, ≥1 upper, ≥1 lower, ≥1 digit, no `"` and
not containing `admin`.

The Telegram bot name is non-sensitive — add it as a **variable** (not a secret):

| Variable | Used for |
| --- | --- |
| `TELEGRAM_BOT_NAME` | Telegram bot username, e.g. `OCI_TEAM31_BOT` (`app-env`) |

`OPEN_ROUTER_API_KEY`, `TELEGRAM_BOT_TOKEN` and `TELEGRAM_BOT_NAME` are written
into the `app-env` Kubernetes secret by `bootstrap-app.sh` and reach the app via
the manifest's `envFrom`. `JWT_SECRET` is **not** configured — the bootstrap
generates one on the first deploy and reuses it on later deploys (stable across
the 2 replicas and restarts).
...
OPEN_ROUTER_API_KEY   TELEGRAM_BOT_TOKEN
# Variables (not secrets):
TELEGRAM_BOT_NAME
# Auto-generated by the deploy job, do NOT set: JWT_SECRET
```

Add them under **Settings → Environments → `prod`** (the workflows use
`environment: prod`) or as repo secrets. `TELEGRAM_BOT_NAME` goes under the
**Variables** tab of the same screen.
