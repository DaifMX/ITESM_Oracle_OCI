...
| `OPEN_ROUTER_API_KEY` | OpenRouter API key for the AI assistant (`/ask`) |
| `TELEGRAM_BOT_TOKEN` | Token for your Telegram bot (from @BotFather) |

The Telegram **bot name** is non-sensitive, so add it as a repo/environment **variable** (Settings → Variables), not a secret:

| Variable | Description |
|---|---|
| `TELEGRAM_BOT_NAME` | The bot's username, e.g. `OCI_TEAM31_BOT` |

> [!NOTE]
> `JWT_SECRET` is **not** a secret you set. The deploy job generates one automatically on the first deploy and reuses it on every later deploy, so sessions stay valid across pods and restarts.
...
- **OpenRouter key:** [openrouter.ai](https://openrouter.ai/keys) → **Create Key** → `OPEN_ROUTER_API_KEY`.
- **Telegram token & name:** message [@BotFather](https://t.me/BotFather) → `/newbot` (or `/token` for an existing bot). It returns the token (`TELEGRAM_BOT_TOKEN`) and the bot's username (`TELEGRAM_BOT_NAME`).
